...
//  Snoo
//
//  Created by Powermobile on 18-06-15.
...
class Keychain {
    ...
    static var serviceName = "com.madePowermobile.beam"
    ...
    static func save(_ key: String, data: Data) throws {
        ...
        if status != noErr {
            if let error = KeychainError(rawValue: status) {
                ...
                throw NSError.beamError(Int(status), localizedDescription: "Unknown keychain save error (\(status))")
            }
        }
    }
    ...
    static func load(_ key: String) throws -> Data? {
        ...
        if status == noErr {
            ...
            if let error = KeychainError(rawValue: status) {
                ...
                throw NSError.beamError(Int(status), localizedDescription: "Unknown keychain load error (\(status))")
            }
        }
    }
    ...
    static func delete(_ key: String) throws {
        ...
        if status != noErr {
            if let error = KeychainError(rawValue: status) {
                ...
                throw NSError.beamError(Int(status), localizedDescription: "Unknown keychain delete error (\(status))")
            }
        }
    }
    ...
    static func clear() throws {
        ...
        if status != noErr {
            if let error = KeychainError(rawValue: status) {
                ...
                throw NSError.beamError(Int(status), localizedDescription: "Unknown keychain clear error (\(status))")
            }
        }
    }
