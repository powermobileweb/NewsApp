...
//  beam
//
//  Created by Powermobile on 20-11-15.
...
extension UIImage {
    ...
    fileprivate class func destinationImageSizeWithOriginalSize(_ size: CGSize, constrainingSize: CGSize, contentMode: UIViewContentMode) -> CGSize {
        ...
        let widthConstrainedSize = CGSize(width: constrainingSize.width, height: constrainingSize.width / sourceRatio)
        let heightConstrainedSize = CGSize(width: constrainingSize.height * sourceRatio, height: constrainingSize.height)
    ...
    @objc public class func downscaledImageWithFileURL(_ fileURL: URL, constrainingSize: CGSize, contentMode: UIViewContentMode) -> UIImage? {
        ...
        autoreleasepool {
            ...
            let destSize = self.destinationImageSizeWithOriginalSize(sourceResolution, constrainingSize: constrainingSize, contentMode: contentMode)
            
            let destWidth = floor(destSize.width * UIScreen.main.scale)
            let destHeight = floor(destSize.height * UIScreen.main.scale)
